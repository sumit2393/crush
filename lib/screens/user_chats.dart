...
import 'package:crush/models/user.dart';
import 'package:firebase_messaging/firebase_messaging.dart';
import 'package:flutter/cupertino.dart';
import 'package:crush/utils/permissions.dart';
import 'package:flutter/material.dart';
import 'package:crush/models/chats.dart';
import 'package:flutter/rendering.dart';
import 'package:crush/utils/functions.dart';
import 'package:flutter_local_notifications/flutter_local_notifications.dart';
import 'package:fluttertoast/fluttertoast.dart';
import 'package:crush/widgets/profileImageWidget.dart';
import 'package:crush/widgets/videoCall_widget.dart';
import 'package:agora_rtc_engine/agora_rtc_engine.dart';
import 'package:crush/utils/constants.dart';
import 'package:crush/widgets/cached_image.dart';
import 'package:crush/screens/payment_screen.dart';
...
class ChatScreen extends StatefulWidget {
  ...
  final String status;
//
  ChatScreen({this.senderId, this.user, this.sender, this.status, this.firebaseMessaging, this.block, this.flutterLocalNotificationsPlugin});
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    addUserToContacts(widget.user.userId, widget.senderId, widget.sender.username);
    addUserToContacts(widget.senderId, widget.user.userId, widget.user.username);
    update();
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(widget.sender.username, style: TextStyle(fontWeight: FontWeight.w600, color: Colors.white)),
                          SizedBox(height: 6.0,),
                          Text(status, style: TextStyle(fontSize: 12.0, color: Colors.white),)
                        ],
                      ),
                    )
                  ],
            ...
            actions: [
              ...
              PopupMenuButton<bool>(
                child: Padding(
                    ...
                    child: Icon(Icons.more_vert, size: 25.0, color: Colors.white)
                  ),
                ...
                itemBuilder: (context) {
                  return <PopupMenuEntry<bool>>[
                    PopupMenuItem(
                      child: Container(
                          width: width * 0.25,
                          child: !block?Text("Block User", style: TextStyle(fontSize: 16.0)):Text("Unblock User", style: TextStyle(fontSize: 16.0))
                        ),
                      value: true,
                    )
                  ];
        ...
        body: GestureDetector(
            ...
            child: Column(
              children: [
                        ...
                        builder: (context, snapshots) {
                          switch(snapshots.connectionState) {
                            case ConnectionState.waiting:
                              return Center(child: Text("Loading ...", style: TextStyle(color: Theme.of(context).primaryColor, fontWeight: FontWeight.w500)));
                            default:
                                ...
                                return ListView.builder( 
                                      ...
                                      itemBuilder: (BuildContext context, int index) {
                                        return MessageBubble(
                                            ...
                                            text: snapshots.data.documents[index]["content"],
                                            isMe: snapshots.data.documents[index]["from"] == widget.user.userId,
                                          );
                                      },
                                );
                              } else {
                ...
                !block?buildMessageComposer():Container(
                  color: Colors.white,
                  child: Padding(
                    padding: EdgeInsets.only(left: width * 0.05, right: width * 0.02, bottom: height * 0.01),
                    child: Text("User has been blocked, unblock from the above options", style: TextStyle(color: Colors.black26, fontSize: 25.0), overflow: TextOverflow.clip))
                )
              ],
            ),
          ),
                          ...
                          if (i<stickers.length) {
                            ...
                            onPressed: () {
                              if (coins > int.parse(stickerCoins[i])) {
                                ...
                                Fluttertoast.showToast(
                                  msg: "You have no coins left in your account, Please buy more coins to send more messages.",
                                  gravity: ToastGravity.BOTTOM
                                );
                              }
                            },
                            child: Column(
                              children: [
                                ...
                                Padding(
                                    ...
                                    child: Row(
                                    children: [
                                      Icon(Icons.monetization_on, size: 15.0, color: Colors.yellow),
                                      SizedBox(width: 4.0),
                                      Text(stickerCoins[i], style: TextStyle(color: Colors.black))
                                    ],
                                  ),
                                )
                              ],
...
//                      }
      ...
      child: Material(
        ...
        child: Row(
          children: [
            ...
            IconButton(
              icon: !isShowStickers?Image(image: AssetImage(gift_box)):Icon(Icons.close),
              iconSize: 25.0,
  ...
  void initializeStickers() {
    stickerCoins = [
      "2500", "10000", '1000', '75', '300', '5000', '750', 
      '2000', '50', '4000', '500', '3000', '30', '150', '1500'
    ];
    bottomButton = [
      RawMaterialButton(
        elevation: 0.0,
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              SizedBox(width: 2.0),
              Text("$myCoin My Coins", style: TextStyle(color: Colors.white, fontSize: 15.0))
            ],
          ),
        ),
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              Icon(Icons.monetization_on, color: Colors.yellow, size: 25.0),
              SizedBox(width: 2.0),
              Text("Purchase", style: TextStyle(color: Colors.white, fontSize: 15.0))
            ],
          ),
        ),
        shape: RoundedRectangleBorder(),
    ...
    stickers = [
        bracelet, car, champagne, chocolate, cigar, crown, 
        heart_gift_box, heels, lollipop, necklace, perfume,
        ring, rose, rose_box, treasure 
    ];
  }
