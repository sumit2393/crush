...
class VideoCallingState extends State<VideoCalling> {
  ...
  void initState() {
    ...
    if (widget.receiver.userId != widget.user.userId) {
      ...
      addUserToContacts(
          widget.user.userId, widget.receiver.userId, widget.receiver.username);
    }
    getCoins();
    initialize();
  ...
  void notify(User receiver) async {
    await Firestore.instance
        ...
        .setData({
      'content': '${widget.user.username} is calling you',
      'receiverToken': receiver.token,
      'title': "You have a notification from ${widget.user.username}",
      'senderName': widget.user.username,
      'senderImageUrl': widget.user.imageUrl,
      'senderCountry': widget.user.country,
